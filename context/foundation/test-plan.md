...
| #2 | Total calories across all meals in a generated set sum to ≤ daily target | "The prompt says stay under X kcal" — same LLM compliance issue | How calorie target flows into prompt; how per-meal calories are returned; whether they're validated post-generation | Integration test (generate with target, sum calories, assert ≤ target) | Asserting the prompt mentions the number — template test, not calorie test |
...
|---|-----------|-----------------|---------------|------------|--------|---------------|
| 1 | Critical-path generation constraints | Prove disallow-list and calorie limits are enforced on LLM output | #1, #2 | unit + integration | change opened | context/changes/testing-critical-path-constraints/ |
...
TBD — see §3 Phase 1 for generation constraint validation patterns and §3 Phase 2 for supply deduction math.

### 6.2 Adding an integration test

TBD — see §3 Phase 1 for LLM-output-assertion patterns (disallow-list/calorie enforcement) and §3 Phase 3 for persistence/caching verification.
