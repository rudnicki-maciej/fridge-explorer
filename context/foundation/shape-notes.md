...
checkpoint:
  ...
  gray_areas_resolved:
    ...
    - topic: "persona scope"
      decision: "single user (myself)"
    - topic: "shopping guidance scope"
      decision: "parked — Open Questions"
    - topic: "access control"
      decision: "multi-device sync, minimal auth, single user"
    - topic: "inventory input method"
      decision: "dietary preferences + manual quick-add by category"
    - topic: "timeline"
      decision: "4 weeks, sustained effort acknowledged"
  frs_drafted: 11
...
Existing tools go "plan → specific shopping list → cook". This app reverses the flow: "what I already have → 5 meals/day that hit my calorie target". Shopping guidance, if any, stays at the category level ("stock up on whole grains, vegetables, fruits") rather than demanding specific products — or uses a disallow-list to filter out unwanted items.

The pain is workflow friction combined with a missing capability: no tool currently connects "available ingredients at home" to "balanced daily nutrition plan" without requiring detailed upfront meal planning.
...
### Primary
- The full daily loop works end-to-end: one-time setup (calorie target + dietary preferences) → quick-add supplies after shopping → morning session (pick breakfast, get snack + lunch options) → evening session (pick dinner, get evening snack) — all coordinated and hitting the calorie target.
...
Acknowledged on 2026-05-19: 4-week MVP requires sustained dedication; user accepted.
...
1. **Should the app provide category-level shopping guidance or a disallow-list?** — Parked. Decide after MVP core loop ("fridge → daily plan") is validated. Could be v2.
2. **Resupply nudge: should the app flag when a suggested meal requires items not in current supplies?** — Nice-to-have. No specific quantity tracking in MVP, but could warn based on category-level inventory.
